 ...
 **/
static int bds_state_update(const xmss_params *params,
                             bds_state *state, const unsigned char *sk_seed,
                             const unsigned char *pub_seed,
                             const uint32_t addr[8])
{
    ...
    unsigned int nodeh;
    unsigned int idx = state->next_leaf;
    if (idx >= (1U << params->tree_height)) {
        return -1;
    }
                     ...
                     unsigned char *sk)
{
    ...
    if ((1 + idx_tree) * (1 << params->tree_height) + idx_leaf < (1ULL << params->full_height)) {
        if (bds_state_update(params, &states[params->d], sk_seed, pub_seed, addr) != 0) {
            return -2;
        }
    }
        ...
        if (! (((idx + 1) & ((1ULL << ((i+1)*params->tree_height)) - 1)) == 0)) {
            ...
            if ((1 + idx_tree) * (1 << params->tree_height) + idx_leaf < (1ULL << (params->full_height - params->tree_height * i))) {
                if (i > 0 && updates > 0 && states[params->d + i].next_leaf < (1ULL << params->full_height)) {
                    if (bds_state_update(params, &states[params->d + i], sk_seed, pub_seed, addr) != 0) {
                        return -2;
                    }
                    updates--;
                }
